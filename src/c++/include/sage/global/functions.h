...
//inline double inv_normal_cdf(double m, double s, double p) { return m + s*ndtri(p); }
...
// from numerics/normal_pdf.h
inline double normal_pdf(double mean, double var)
{
...
inline double normal_pdf(double z, double mean, double var)
{
  //double sqrt_component = 1.0 / sqrt(2.0 * numeric_constants<double>::pi() * var);
  ...
  return normal_pdf((z - mean), var);
}

inline log_double log_normal_pdf(double mean, double var)
{
  //double sqrt_component = 1.0 / sqrt(2.0 * numeric_constants<double>::pi() * var);
  //double exp_component = exp( -0.5 * mean * mean / var );

  //return log_double(sqrt_component * exp_component);
  return log_double(normal_pdf(mean, var));
}
