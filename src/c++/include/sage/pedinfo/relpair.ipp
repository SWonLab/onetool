...
{
  out << "pair type:  " << std::setw(10) << std::left << pair_generator::pair_type_to_string(pr.type()) << "      ";
...
inline
pair_generator::pair_generator(RefPedigree* p, unsigned int types) 
...
inline void 
pair_generator::set_pedigree(RefPedigree* pedigree)
{
  my_p = pedigree;
}
...
{
  std::unique_ptr<pair_generator>  ap(new pair_generator(*(other.my_generator)));
  my_generator            = std::move(ap);
...
pair_generator::base_iterator::base_iterator(pair_generator* generator,
      ...
      : my_family(family)
{
  std::unique_ptr<pair_generator> ap(new pair_generator(*generator));
  my_generator = std::move(ap);
