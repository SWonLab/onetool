...
class PedigreeDataSet
{
  public:
    ...
    std::shared_ptr<const FPED::Multipedigree> get_raw_data() const;
  ...
  private:
    ...
    void report_empty_dataset_error  (Output_Streams& output);
    void report_dataset_sex_errors   (const model&    model,
                                      Output_Streams& output);
    void report_transmission_error   (Output_Streams& output);

    std::shared_ptr<FPED::Multipedigree> my_data;
...
#include "sage/segreg/PedigreeDataSet.ipp"
