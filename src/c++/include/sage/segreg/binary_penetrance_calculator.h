...
class BinaryPenetranceCalculator : public std::binary_function<double, const MEMBER_TYPE&, const GENOTYPE&>
{
  ...
  private:
    ...
    typedef std::function<double (const MEMBER_TYPE&, const GENOTYPE&)> SuscFuncType;
    typedef std::function<double (const MEMBER_TYPE&)>                  AffFuncType;
...
#include "sage/segreg/binary_penetrance_calculator.ipp"
