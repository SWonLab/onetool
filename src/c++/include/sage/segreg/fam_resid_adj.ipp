...
#include "sage/segreg/fam_resid_adj.h"
#endif
   ...
   const Genotype&   father_genotype) const
{
  ...
  for(typename MPTYPE::offspring_const_iterator child = fam.offspring_begin();
      child != fam.offspring_end(); ++child, ++i)
  {
    child_data[i].penetrance  = this->calc_est_penetrance(*child, mother_genotype, father_genotype);
    child_data[i].affection   = this->my_aff_func(*child);
    
    if(isnan(child_data[i].penetrance)) child_data[i].penetrance = 1.0;
  }
  
  return this->calc_adjustment(fam, mother_genotype, father_genotype, child_data);
}
...
{
  double mean = this->calc_est_susceptibility(mem,mgeno,fgeno);
  
  bool affection = this->my_aff_func(mem);
   ...
   const Genotype&   fgeno) const
{
  ...
  { 
    ...
    double penetrance     = this->my_pen_func    (mem, (Genotype) *gtype);
