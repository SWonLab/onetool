...
#include "merlin/libsrc/PedigreeDescription.h"
#include "merlin/libsrc/MapFunction.h"
#include "merlin/libsrc/MathVector.h"
#include "merlin/libsrc/Constant.h"
...
void PedigreeDescription::Load(ONETOOL::cIO* Cp_IO)
{
	// Check if we are dealing with a linkage format data file
	String      buffer;
	...
	ONETOOL::vPheno&	Xv_phe	= Cp_IO->getPhenoInfo();
	char*				Ba_cont	= Cp_IO->getTypePheno();
	ONETOOL::vCovar&	Xv_cov	= Cp_IO->getCovInfo();
	ONETOOL::vVariant&	Xv_vrt	= Cp_IO->getVariant();

	FOREACHDO (ONETOOL::vPheno_it, Xv_phe, it, line++) {
		if (Ba_cont[line]) {
			/* TYPE T */
			columnHash.Push(GetTraitID(it->S_name.c_str()));
			columns.Push(pcTrait);
			columnCount++;
		} else {
			/* TYPE A */
			columnHash.Push(GetAffectionID(it->S_name.c_str()));
			columns.Push(pcAffection);
			columnCount++;
		}
	}
	line = 0;
	FOREACHDO (ONETOOL::vCovar_it, Xv_cov, it, line++) {
		/* TYPE C */
		columnHash.Push(GetCovariateID(it->Sp_varName));
		columns.Push(pcCovariate);
		columnCount++;
	}
	line = 0;
	FOREACHDO (ONETOOL::vVariant_it, Xv_vrt, it, line++) {
		/* TYPE M */
		columnHash.Push(GetMarkerID(it->name));
		columns.Push(pcMarker);
		columnCount++;
	}
