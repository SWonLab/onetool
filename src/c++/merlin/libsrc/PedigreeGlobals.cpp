...
#include "merlin/libsrc/PedigreeGlobals.h"
#include "merlin/libsrc/Sort.h"
...
void PedigreeGlobals::LoadMarkerMap(ONETOOL::cIO* Cp_IO, bool filter)
{
	ONETOOL::vVariant&	Xv_vrt = Cp_IO->getVariant();
   bool first_pass = true;
   sexSpecificMap = false;

   FOREACH (ONETOOL::vVariant_it, Xv_vrt, it) {
	  ...
	  // 1 = marker name
	  // 2 = pos
	  String chrS = ONETOOL::getChrName2(it->chr);
	  String name = it->name;
	  double pos = it->gdist;
      ...
      if (filter)
         if (LookupMarker(name) < 0)
            continue;

      MarkerInfo * info = GetMarkerInfo(name);

      int chr = (chrS[0] == 'x' || chrS[0] == 'X') ? 999 : (int) chrS;

      info->chromosome = chr;
      info->position = (double) pos * 0.01;
