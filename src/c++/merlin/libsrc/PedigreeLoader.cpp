...
#include "merlin/libsrc/FortranFormat.h"
#include "merlin/libsrc/Error.h"
...
// Read pedigree from @ONETOOL
void Pedigree::Prepare(ONETOOL::cIO* Cp_IO)
{
	pd.Load(Cp_IO);
}
...
// Read pedigree from @ONETOOL
void Pedigree::Load(ONETOOL::cIO* Cp_IO)
{
	ONETOOL::vSampPtr& Xv_samp = Cp_IO->getSample();
	String* tokens = new String[16];
	
	int sexCovariate = sexAsCovariate ? GetCovariateID("sex") : -1;
	
	ONETOOL::wsMat Ra_pheno = Cp_IO->getPhenos();
	ONETOOL::wsMat Ra_cov   = Cp_IO->getCovariates();
	char** Na_geno = Cp_IO->getGenotype();

	int oldCount = count;
	int idx = 0;
	FOREACHDO (ONETOOL::vSampPtr_it, Xv_samp, i, idx++) {
		int N_pidx = 0;
		int N_cidx = 0;
		int N_midx = 0;
		
		int field = 0;
		tokens[0] = (char *)(*i)->S_FID.c_str();
		tokens[1] = (char *)(*i)->S_IID.c_str();
		tokens[2] = (char *)((*i)->Xp_pat ? (*i)->Xp_pat->S_IID.c_str() : "0");
		tokens[3] = (char *)((*i)->Xp_mat ? (*i)->Xp_mat->S_IID.c_str() : "0");
		tokens[4] = (char *)((*i)->N_sex == 2 ? "2" : ((*i)->N_sex == 1 ? "1" : "0"));
      ...
      for (int col = 0; col < pd.columnCount; col++)
         switch ( pd.columns[col] )
            ...
            case pcAffection :
               {
               int a = pd.columnHash[col];
               ONETOOL::wsReal new_status;
			   
			   new_status = Ra_pheno[N_pidx++][idx];
//			   ONETOOL::LOG("status %g to %d\n", new_status, a);
			   if (new_status != ONETOOL::WISARD_NA_REAL)
				   p->affections[a] = new_status == WISARD_AFFECTED ? 2 : 1;
               break;
               }
            case pcMarker :
               {
			   ...
			   char N_geno = Na_geno[idx][N_midx];
			   String a = N_geno>0 ? "2" : (isMissing(N_geno) ? "0" : "1");
			   String b = N_geno>1 ? "2" : (isMissing(N_geno) ? "0" : "1");
			   N_midx++;

               new_genotype[0] = LoadAllele(m, a);
               new_genotype[1] = LoadAllele(m, b);

//			   ONETOOL::LOG("genotype %d %d\n", new_genotype[0], new_genotype[1]);
            ...
            case pcUndocumentedTraitCovariate :
			   {
			   ONETOOL::halt("This type cannot be handled in ONETOOLxMERLIN");
			   break;
			   }
            case pcTrait :
               {
               int t = pd.columnHash[col];
               double new_pheno = Ra_pheno[N_pidx++][idx];
//			   ONETOOL::LOG("trait %g\n", new_pheno);
			   if (new_pheno != ONETOOL::WISARD_NA_REAL)
				   p->traits[t] = new_pheno;
               if (pd.columns[col] == pcTrait) break;
               }
            case pcCovariate :
               {
               int c = pd.columnHash[col];
               double new_covar = Ra_cov[N_cidx++][idx];

               if ( new_covar != ONETOOL::WISARD_NA_REAL)
				   p->covariates[c] = new_covar;
               break;
               }
            case pcSkip :
