...
#include "merlin/libsrc/TraitTransformations.h"
#include "merlin/merlin/MerlinSimulator.h"
#include "merlin/merlin/MerlinCluster.h"
#include "merlin/merlin/MerlinFamily.h"
#include "merlin/merlin/MerlinError.h"
#include "merlin/merlin/MerlinSort.h"
#include "merlin/merlin/Manners.h"
#include "merlin/libsrc/Error.h"

#include "merlin/libsrc/LongInt.h"

// @ONETOOL main routine fetched by ONETOOL
int main_ONETOOL(ONETOOL::cIO* Cp_IO, int N_argc, char* Sa_argv[])
{
	...
	MerlinParameters pl;
	pl.Read(N_argc, Sa_argv, 1);
	pl.Status();

	ped.LoadMarkerMap(Cp_IO, true /* filter out markers not in pedigree */);
//@	ped.LoadAlleleFrequencies(pl.freqFile);

	// @ONETOOL this step is previously done by ONETOOL itself
//   if (pl.inverseNormal)
