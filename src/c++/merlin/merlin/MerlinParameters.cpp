...
#include "merlin/merlin/MerlinParameters.h"
#include "merlin/merlin/MerlinSimulator.h"
#include "merlin/merlin/FastAssociation.h"
#include "merlin/merlin/MerlinFamily.h"
#include "merlin/merlin/MerlinModel.h"
#include "merlin/merlin/MerlinCache.h"
#include "merlin/clusters/HaploFamily.h"
#include "merlin/merlin/KongAndCox.h"
#include "merlin/libsrc/Pedigree.h"
#include "merlin/merlin/Houdini.h"
#include "merlin/libsrc/Random.h"
...
MerlinParameters::MerlinParameters() :
   ...
   random_seed(123456)
   {
   Add(new MERLIN::StringParameter('d', "Data File", dataFile));
   Add(new MERLIN::StringParameter('p', "Pedigree File", pedigreeFile));
   Add(new MERLIN::StringParameter('x', "Missing Value Code", Pedigree::missing, false));
   Add(new MERLIN::StringParameter('m', "Map File", mapFile));
   Add(new AlleleFrequencyParameter('f', "Allele Frequencies", alleleFrequencies, freqFile));
   Add(new MERLIN::IntParameter('r', "Random Seed", random_seed));
   Add(new MERLIN::LongParameters("Data Analysis Options", longParameters));

   // The -b switch is obsolete and is thus hidden (use --bits instead)
   Add(new MERLIN::HiddenInteger('b', "Maximum Bits", FamilyAnalysis::maxBits));
   // The -i switch is obsolete and is thus hidden (use --steps instead)
   Add(new MERLIN::HiddenInteger('i', "Steps Per Interval", FamilyAnalysis::steps_per_interval));
   }
...
void MerlinParameters::Read(int argc, char ** argv, int start)
   ...
   if (FuzzyInheritanceTree::perAlleleError != 0.0 || 
       ...
       fitErrorRate)
      {
      char * reason = (char *)"The --%s option is disabled when error modeling is enabled";
   ...
   if (FamilyAnalysis::twopoint)
      {
      char * reason = (char *)"The %s option is disabled during "
   ...
   if (FamilyAnalysis::allHaplotypes)
      {
      char * reason = (char *)"The --%s option is disabled when listing all haplotypes\n";
   ...
   if (FamilyAnalysis::simwalk2)
      ...
      const char * reason2 = (char *)" %s option enabled when creating "
                             "intermediate results for SimWalk2\n";
