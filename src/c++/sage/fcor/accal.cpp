...
#include "sage/global/matfunctions.h"
#include "sage/fcor/accal.h"
                          ...
                          Matrix2D< vector<size_t> >&      lp_matrix) const
{
  ...
  size_t trait_count = my_correlation->get_mped()->info().trait_count();
  ...
  vector<double> ac(wcnt, SAGE::QNAN);
    ...
    for( size_t i = 0; i < ac_matrix.rows(); ++i )
      for( size_t j = 0; j < ac_matrix.cols(); ++j )
        for( size_t w = 0; w < ac_matrix(i,j).size() ; ++w )
        {
          ...
          if( !(SAGE::isnan(ac_matrix1(i,j)[w]) || SAGE::isnan(ac_matrix2(i,j)[w])) )
          {
    ...
    for( size_t i = 0; i < ac_matrix.rows(); ++i )
      for( size_t j = 0; j <= i; ++j )
        for( size_t w = 0; w < ac_matrix(i,j).size() ; ++w )
        {
          ...
          if( !(SAGE::isnan(ac_matrix(i,j)[w]) || SAGE::isnan(ac_matrix(j,i)[w])) )
          {
...
StdErrCal::compute_standard_errors(pairset_result_vector& results)
{
  ...
  weight_type w           = PAIR_WISE;
  size_t generation_limit = 3;
  size_t trait_count      = my_correlation->get_mped()->info().trait_count();
  size_t matrix_size      = trait_count * trait_count;
  ...
  for( size_t r = 0; r < pairsets.size(); ++r )
  {
    ...
    if( pinfos[r].type == OTHER && my_correlation->get_mped()->info().trait_count() > 1 )
    {
      Matrix2D<double> new_ac_matrix;
      new_ac_matrix.resize(matrix_size, matrix_size, SAGE::QNAN);
      ...
      //{
        for( size_t t1 = 0; t1 < ac_matrix.rows(); ++t1 )
          for( size_t t2 = 0; t2 < ac_matrix.cols(); ++t2 )
            if( !SAGE::isnan(ac_matrix(t1, t2)[w]) )
            {
                                            ...
                                            const Matrix2D< double >& ac_matrix) const
{
  ...
  if( trait_count > 2 )
  {
    Matrix2D<double> x_y(df, 1, SAGE::QNAN);
    Matrix2D<double> Vx_y(df, df, SAGE::QNAN);
                                              ...
                                              var_cov_result_vector& vc_results)
{
  ...
  size_t trait_count = my_sub_correlation->get_mped()->info().trait_count();
  size_t matrix_size = trait_count * trait_count;
                                       ...
                                       const CorrelationCal*        corcal)
{
  ...
  if( get_analysis_options().class_weight != WEIGHT_COUNT )
  {
  ...
  else
  {
    ...
    optimal_ac_matrix.resize(matrix_size, matrix_size, SAGE::QNAN);
