...
CorrelationCal::CorrelationCal(const RefMultiPedigree* mp)
{
  my_valid        = false;
  my_mped         = mp;
  my_pairset      = NULL;
...
{
  if( my_mped == NULL || my_pairset == NULL)
    return;

  bool se = true;
  size_t trait_count = my_mped->info().trait_count();
  ...
  for( size_t r = 0; r < my_pairset->size(); ++r )
  {
    ...
    if( (*my_pairset_info)[r].total_pair_count > 2 )
    {
      ...
      for( size_t p = 0; p < pairset.size(); ++p )
      {
        ...
        tri_weight.resize(trait_count * 2);
        ...
        for( size_t t1 = 0; t1 < tri_weight.size(); ++t1 )
        {
          ...
          {
            tri_weight(t1, t2) = ped_weight.first(t1%trait_count, t2%trait_count);
        ...
        {
          vector<double> traits(trait_count * 2);

          const_pedigree_member_pair m_pair = pairset[p][i].member_pair;

          for( size_t t = 0; t < trait_count; ++t )
          {
            traits[t] = m_pair.first->pedigree()->info().trait(m_pair.first ->index(), t);
          }

          for( size_t t = 0; t < trait_count; ++t )
          {
            traits[t + trait_count] = m_pair.second->pedigree()->info().trait(m_pair.second->index(), t);
          }
...
{
  size_t ped_count = my_mped->pedigree_count();
  my_corinfo_map.resize(ped_count);

  for( size_t p = 0; p < ped_count; ++p )
    my_corinfo_map[p].resize(my_mped->pedigree_index(p).member_count());
