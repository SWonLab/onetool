...
FcorView::FcorView(const analysis_option_type& fo, const PairSetData& pairs)
        : my_options(fo)
{
...
FcorView::print_analysis_header(ostream& o, pair_type ptype, output_type otype) const
{
  ...
  //
  o << endl;
  o << "    Number of pedigrees = " << my_pairsetdata->get_mped()->pedigree_count() << endl;
  ...
  for( size_t t1 = 0; t1 < trait_count(); ++t1 )
  {
    ...
    if( get_analysis_options().standard_error )
    {
      ...
      if( selfclass )
      {
        ...
        print_standard_error(o,        SAGE::QNAN, 6, 1,
                             "   +/-", SAGE::QNAN, 7, 4, false);
        o << "           ";
...
FcorView::print_pooled_corr(ostream& o, const pairset_result& cor_std) const
{
  ...
  if( !SAGE::isnan(chsq) && !SAGE::isnan(pval) )
  {
    o << endl
      << " ------------------------------------------" << endl
