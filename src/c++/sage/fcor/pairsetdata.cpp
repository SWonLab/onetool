...
#include "sage/fcor/pairsetdata.h"
...
void
PairSetData::build_pairset(const RefMultiPedigree* mp)
{
  my_mped = mp;
  RelationMatrix  relmatrix(mp);

  build_pairset(relmatrix, mp);
}
                           ...
                           const RefMultiPedigree* mp)
{
  ...
  //
  for( size_t k = 0; k < my_mped->pedigree_count(); ++k )
  {
    const RefPedigree& sp = my_mped->pedigree_index(k);

    size_t rank = sp.member_count();

    const RefPedInfo& pinfo = sp.info();
    ...
    {
      for(size_t t = 0; t < my_mped->info().trait_count(); ++t)
        if( finite( pinfo.trait(i, t) ) )
        {
          informative[i] = true;
          break;
        }
    ...
    for( size_t i = 0; i < rank ; ++i )
    {
      ...
      for( size_t j = 0; j <= i; ++j )
      {
        ...
        {
          sp_map[c].resize(my_mped->pedigree_count());
          sc_map[c] = 0;
        }
...
PairSetData::build_main_pairset()
{
  ...
  for( ; rel_group != rel_group_end; ++rel_group )
  {
    ...
    main_set.resize(my_mped->pedigree_count());
