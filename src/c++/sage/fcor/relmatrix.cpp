...
RelationMatrix::generate_matrix()
{
  ...
  {
    const RPED::pedigree_type* p = &mp->pedigree_index(ped);
  ...
  for( size_t k = 0; k < my_relmatrix.size(); ++k )
  {
    ...
    for( size_t i = 0; i < rank ; ++i )
    {
      ...
      for( size_t j = 0; j <= i; ++j )
        if(raw)
          out << std::setw(10) << my_relmatrix[k](i,j).str()              << "|";
        else
          out << std::setw(10) << relationship_name(my_relmatrix[k](i,j)) << "|";
      out << std::endl;
    }
  }
}
...
void
RelationMatrix::view_pairs(std::ostream& out, const RPED::RefMultiPedigree& mp) const
{
  ...
  {
    out << std::setw(12) << relationship_name(rel_type->first) << " [n="
        << std::setw(3)  << std::right << rel_type->second.size() << "]: " << std::left;
    ...
    {
      if( k && k % 10 == 0 )
        out << std::endl << std::setw(22) << "";
      out << '(' << std::setw(2) << p->first->name()
          << ',' << std::setw(2) << p->second->name() << ") ";
    }
    out << std::endl;
  }
}
...
void
RelationMatrix::view_pairs_gender(std::ostream& out, const RPED::RefMultiPedigree& mp) const
{
  ...
  for( ; rel_type != rel_end; ++rel_type )
  {
    ...
    out << std::setw(9) << gender_list;

    out << " [n=" << std::setw(3) << std::right << rel_type->second.size() << "]: " << std::left;
...
void
RelationMatrix::view_pairs_xml(std::ostream& out, const RPED::RefMultiPedigree& mp) const
{
  typedef   LinearDatamap<RelationType,
                          const_pedigree_member_pair,
                          RelationMateLess>          relation_map;
  ...
  out << "<?xml version=\"1.0\"?>" << endl
      << "<?mso-application progid=\"Excel.Sheet\"?>" << endl
      << "<Workbook" << endl
  ...
  out << "  <Worksheet ss:Name=\"relpair info\">" << endl
      << "    <Table>" << endl;

  out << "      <Row>" << endl
      << "        <Cell>" << endl
      << "          <Data ss:Type=\"String\">" << "sheet no." << "</Data>" << endl
      << "        </Cell>" << endl
      << "        <Cell>" << endl
      << "          <Data ss:Type=\"String\">" << "pair code " << "</Data>" << endl
      << "        </Cell>" << endl
      << "        <Cell>" << endl
      << "          <Data ss:Type=\"String\">" << "pair description" << "</Data>" << endl
      << "        </Cell>" << endl
      << "        <Cell>" << endl
      << "          <Data ss:Type=\"String\">" << "pair count" << "</Data>" << endl
      << "        </Cell>" << endl
      << "      </Row>" << endl;

  size_t pt = 1;
  for( ; rel_type != rel_end; ++rel_type, ++pt )
  {
    out << "      <Row>" << endl
        << "        <Cell>" << endl
        //<< "          <Data ss:Type=\"String\">" << relationship_number(rel_type->first) << "</Data>" << endl
        << "          <Data ss:Type=\"Number\">" << pt << "</Data>" << endl
        << "        </Cell>" << endl
        << "        <Cell>" << endl
        << "          <Data ss:Type=\"String\">" << relationship_name(rel_type->first) << "</Data>" << endl
        << "        </Cell>" << endl
        << "        <Cell>" << endl
        << "          <Data ss:Type=\"String\">" << long_relationship_name(rel_type->first) << "</Data>" << endl
        << "        </Cell>" << endl
        << "        <Cell>" << endl
        << "          <Data ss:Type=\"Number\">" << rel_type->second.size() << "</Data>" << endl
        << "        </Cell>" << endl
        << "      </Row>" << endl;
  }
  out << "    </Table>" << endl
  ...
  rel_type = relpairs.type_begin();
  pt = 1;
  for( ; rel_type != rel_end; ++rel_type, ++pt )
  {
    //out << "  <Worksheet ss:Name=\"" << relationship_number(rel_type->first) << "\">" << endl
    out << "  <Worksheet ss:Name=\"" << pt << "\">" << endl
        << "    <Table>" << endl;
    ...
    for( size_t k = 0; p != pair_end; ++p, ++k )
    {
      out << "      <Row>" << endl
          << "        <Cell>" << endl
          << "          <Data ss:Type=\"String\">" << p->first->name() << "</Data>" << endl
          << "        </Cell>" << endl
          << "        <Cell>" << endl
          << "          <Data ss:Type=\"String\">" << p->second->name() << "</Data>" << endl
          << "        </Cell>" << endl
          << "      </Row>" << endl;
    }
    out << "    </Table>" << endl
