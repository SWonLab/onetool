#include "sage/gelim/inconsistency_handler.h"
  ...
  for(handler::fam_iterator l = incon_id_list.begin(); l != incon_id_list.end(); ++l)
  {
    ...
    if( mendelian_exist )
    {
      ...
      for( ; mfam != subped.family_end(); ++mfam )
      {
        ...
        {
          print_inconsistency_member(output, *(mfam->parent2()), mendelian_found,
                                     imap, start_col, end_col, 0, sample_id);

          print_inconsistency_member(output, *(mfam->parent1()), mendelian_found,
