#include "sage/gelim/ped_imodel_gen.h"

#undef DEBUG
//#define DEBUG_GELIM 1
...
pedigree_imodel_generator::operator()(const subpedigree& sped, size_t marker) const
{
...
#ifdef DEBUG_GELIM
  cout << "1. original model:" << endl;
  MLOCUS::inheritance_model_test(cout, orig, orig.name().c_str());
  MLOCUS::inheritance_model_print(cout, orig, orig.name().c_str());
#endif
  ...
  cout << ind  << ": sped_index = " << sped_index
       << ", index = " << ped_index
       << ", name = " << sped.member_index(ind).name()
       << ", pheno = " << sped.member_index(ind).info().phenotype(marker)
       << endl;
#endif
...
#ifdef DEBUG_GELIM
  cout << "new model:" << endl;
  MLOCUS::inheritance_model_test(cout, model, model.name().c_str());
  MLOCUS::inheritance_model_print(cout, model, model.name().c_str());
#endif
