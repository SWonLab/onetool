...
//#include "sage/global/LSFfactory.h"
...
LSFBase *LSF_input::getLSF(bool typed)
{
  ...
  name = getStringSAGE( *lsf_file, "=,#{}<>;\n\r", " \t" );

  if( name.length() && !(isalnum(name[0]) || strchr("$_",name[0])) )
  {
    error((char *)"Invalid LSF name");
  ...
  {
    error((char *)"Unexpected character");
  ...
  if(typed)
  {
    ...
    type = getStringSAGE( *lsf_file, "=,#;{}<>\n\r", " \t" );

    if( type.length() && !isalpha(type[0]) )
    {
      error((char *)"Invalid LSF type");
  ...
  if (!lsf_file->eof())
  {
    ...
    if( !strchr( "\n\r#;$,={}<>", c ) )
    {
      error((char *)"Unexpected character in LSF argument list");
    ...
    while( !lsf_file->eof() )
    {
      ...
      if(!lsf_file->eof() && lsf_file->peek() == ',')
      {
        ...
        _lines += kill_ws(*lsf_file);
        if( lsf_file->eof() )
          error((char *)"Unexpected end of file");
      }
      else
        break;
    }
                                     ...
                                     const char *ws )
{
  ...
  {
    right = getQStringSAGE( i, delim, ws, "\r\n", &_lines );
    def.set(left, right);
    return;
  }

  right = getStringSAGE( i, delim, ws );
