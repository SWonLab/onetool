...
//=======================================================================
Results
Maximizer::MaximizeFunction(ParameterMgr&      info,
                            const SequenceCfg& seq,
                            Maxfun&            maxfun, 
                            ...
                            const DebugCfg&    debug)
{
  ...
  vector<std::shared_ptr<Results> >  results(seq.getRunCfgs().size());
...
// for the sake of checking  
    ...
    results[i] = std::shared_ptr<Results>(new Results(maxfun.get_results(), debug));
