     ...
     cerrorstream&                     errors)
{
  ...
  if(param->List())
  {
    ...
    for(iter = param->List()->begin(); iter != param->List()->end(); ++iter)
    {
      ...
      {
        SAGE::check_for_incorrect_attributes(*iter, exp_list, 6, errors);
      ...
      else if(param_name == "LAMBDA2")
      {
        ...
        SAGE::check_for_incorrect_attributes(*iter, exp_list, 2, errors);
     ...
     cerrorstream&  errors)
{
  ...
  SAGE::LSFConvert::error_t err1 = SAGE::LSFConvert::to_real(param, value);
  SAGE::LSFConvert::error_t err2 = SAGE::LSFConvert::to_real(param, "val", val);

  if(err1 == SAGE::LSFConvert::GOOD && err2 == SAGE::LSFConvert::GOOD)
  {
    errors << priority(error) << "Both value and val attribute set for "
           << name_phrase << ".  Will ignore value." << endl;
    err1 = SAGE::LSFConvert::INVALID;
  }

  switch(err1)
  {
    case SAGE::LSFConvert::GOOD          :
      mi.value = value;
      break;

    case SAGE::LSFConvert::ATTR_INVALID  :
  ...
  SAGE::LSFConvert::error_t fixed_err = SAGE::LSFConvert::to_boolean(param, "FIXED", fixed);
