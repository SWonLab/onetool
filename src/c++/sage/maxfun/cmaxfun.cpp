...
#include "sage/maxfun/maxfun.h"
#include "sage/global/clapack.h"
#include "sage/global/fmatrix.h"
...
int pseudoInverse(double* A, int m, int n)
{
   ...
   F77_CALL(dgelss)(&M, &N, &NRHS, &a[0], &LDA, &b[0], &LDB, SINGV, &RCOND, &IRANK, WORK, &LDWORK, &info);
...
/* --INDICATE TYPE OF APPROXIMATION USED FOR SECOND DERIVATIVES */
    ...
    vector<double> thy(my_data.param_NP);
    vector<double> dthis(my_data.param_NP, SAGE::QNAN);
...
L84:
        ...
        // by setting the delta theta to SAGE::QNAN and then skipping over the
        // later calculations.
...
L360:
   ...
   dthi = dthis[i];
        ...
        if(SAGE::isnan(dthi))
        {
          // Store a SAGE::QNAN in the hessian matrix as well, so that we
          // know this is bad.
...
#define hn_ref(a_1,a_2) hn[(a_2)*my_data.param_NPV + a_1]
    ...
    for (unsigned i = 0; i != my_data.score_vec.size() ; i++){
      if( !(SAGE::isnan(my_data.score_vec[i])) ) count++;
    }
    ...
    for (unsigned i = 0; i != my_data.score_vec.size() ; i++){
      if( !(SAGE::isnan(my_data.score_vec[i])) ) newscorevec.push_back(my_data.score_vec[i]);
    }

   for(unsigned i = 0; i != my_data.score_vec2.size(); i++){
      if( !(SAGE::isnan(my_data.score_vec2[i])) ) newscorevec2.push_back(-1.0*my_data.score_vec2[i]);
   }
...
//  should put check to ensure that newscorevec2 has dimensionality count^{2}
    ...
    if (!(getinverse)) return SAGE::QNAN;
