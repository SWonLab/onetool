...
//===================================================================
// get_max_sibship_size(...)
//===================================================================
uint
multipedigree_base::get_max_sibship_size() const
{
  uint max_sibship_size = 0;

  pedigree_const_iterator ped_itr = pedigree_begin ();
  for( ; ped_itr != pedigree_end(); ++ped_itr )
  {
    family_const_iterator fam_itr  = ped_itr->family_begin();
    for( ; fam_itr != ped_itr->family_end(); ++fam_itr )
    {
      if( fam_itr->offspring_count() > max_sibship_size )
        max_sibship_size = fam_itr->offspring_count();   
    }
  }
