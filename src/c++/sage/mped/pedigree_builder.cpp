...
//
bool
pedigree_builder::add_lineage(const string& child, const string& parent)
{
    pedigree_builder::name_pair   names(parent);
...
//
bool
pedigree_builder::add_marriage(const string& mate1, const string& mate2)
{
    if (mate1.size() > 0  &&  mate2.size() > 0)
    {
        my_marriages.push_back( pedigree_builder::name_pair(mate1, mate2) );
        return true;
    }
    else
    {
...
//
bool
pedigree_builder::add_sibship(const string& sib1, const string& sib2)
{
...
//
void
pedigree_builder::add_lineage(const string& child, const name_pair& src)
{
...
//    uint    ssize1 = src.name1.size(); <- *Not* actually used later, so let's not do it
    ...
    else if (dsize1 != 0  &&  dsize2 == 0)
    {
        if (ssize2 == 0)
        {
            ...
            {
                dst = pedigree_builder::name_pair(dst.name1, src.name1);
            }
        }
        else
        {
...
void
pedigree_builder::build_pedigree(pedigree_base& ped)
{
...
//
void
pedigree_builder::process_marriages(pedigree_base& ped)
{
...
//
void
pedigree_builder::process_sibships(pedigree_base& ped)
{
    uint    count;
...
//
void
pedigree_builder::process_lineages(pedigree_base& ped)
{
...
//
member_id
pedigree_builder::build_sib_chain
  (pedigree_base& ped,
   member_id kid1, 
   member_id par1, 
   sib_chain& chain)
...
//
void
pedigree_builder::build_subpedigrees(pedigree_base& ped)
{
    family_id       f;
...
//
void
pedigree_builder::mark_family(family_id f, subped_id sp)
{
    if (f)
    {
        f->set_subped(sp);
...
//
void
pedigree_builder::mark_all(pedigree_base& ped, subped_id old_id, subped_id new_id)
{
...
//
void
pedigree_builder::build_indices(pedigree_base& ped)
{
    uint    i;
...
/// with this condition.
void
pedigree_builder::infer_sexes(pedigree_base& ped)
{
...
/// error handler.
void
pedigree_builder::check_parental_sex_consistency(pedigree_base& ped)
{
...
/// error handler.
void
pedigree_builder::check_parental_missing_sexes(pedigree_base& ped)
{
...
void
pedigree_builder::assign_arbitrary_sexes(pedigree_base& ped)
{
...
bool
pedigree_builder::assign_arbitrary_sex(member_id ind, member_id mate, SexCode sex,
...
/// error handler.
void
pedigree_builder::test_marriage_loop_consistency(pedigree_base& ped)
{
  // Create a vector of pairs of bools and sexes.  This will keep track of
  // who we visited and what their sex would be to maintain consistency.
  std::vector<std::pair<bool, SexCode> > visit_vector(ped.member_count(), std::make_pair(false, SEX_MISSING));
...
/// \return \c true if the assignment works, \c false if there are problems.
bool
pedigree_builder::test_marriage_loop_consistency
    (member_id ind,
     member_id mate,
     SexCode sex,
     std::vector<std::pair<bool, SexCode> >& visit_vector)
...
void
pedigree_builder::set_family_mother_father(pedigree_base& ped)
{
  for(family_base_iterator fam = ped.family_begin(); fam != ped.family_end(); ++fam)
  {
...
//
void
pedigree_builder::flush_build_info()
{
    my_marriages.clear();
    my_lineages.clear();
    my_sibships.clear();
...
//
void
pedigree_builder::cleanup()
{
    pedigree_builder::marriage_list::iterator     mf = my_marriages.begin();
    pedigree_builder::marriage_list::iterator     ml = my_marriages.end();

    while (mf != ml)
    {
        if (mf->state == pedigree_builder::name_pair::used)
        {
    ...
    pedigree_builder::lineage_map::iterator       lf = my_lineages.begin();
    pedigree_builder::lineage_map::iterator       ll = my_lineages.end();

    while (lf != ll)
    {
        if (lf->second.state == pedigree_builder::name_pair::used)
        {
    ...
    pedigree_builder::sibship_list::iterator      sf = my_sibships.begin();
    pedigree_builder::sibship_list::iterator      sl = my_sibships.end();

    while (sf != sl)
    {
        if (sf->state == pedigree_builder::name_pair::used)
        {
