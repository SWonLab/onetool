...
void produce_output(const primary_analysis_results& a, Output_Streams& o, bool append = false)
{
  ...
  if(a.get_final_model().get_type_prob())
  {
    ...
    o << "Model=\"" << a.get_final_model().get_primary_trait() << "_type\",title=\"" << a.get_final_model().get_title() << "\"" << endl;
    o << "{" << endl;
...
void produce_binary_nt_no_resid_error(const model& md, Output_Streams& o, bool append = false)
{
...
/// the analysis
void
AnalysisController::do_analysis_from_parameter_file(const RPED::MultiPedigree& mp, const string& fname, const string& oname) const
{
  cout << "\n\nReading SEGREG options................................." << endl;
  cout << "               from " << fname << flush;

  ifstream in_state(fname);
  if( !in_state.good() )
  {
    std::cout << std::endl;
    my_out.errors() << SAGE::priority(SAGE::fatal)
                    << "Unable to load parameters....  Exiting." << endl;
  ...
  LSF_ptr<LSFBase> params = new LSFBase("Parameters");
  load_state.input_to(params, false);

  if( !params  || !params->List() )
  {
    std::cout << std::endl;
    my_out.errors() << SAGE::priority(SAGE::fatal)
                    << "Error reading parameter file....  Exiting." << std::endl;

    exit(EXIT_FAILURE);
  }

  parser p(&mp, my_out.messages(), my_out.errors());
  ...
  for( ; i != params->List()->end(); ++i )
  {
    ...
    if( toUpper((*i)->name()) == "SEGREG" || toUpper((*i)->name()) == "SEGREG_ANALYSIS" )
    {
      ...
      {
        model target_model = p.get_model();
        target_model.set_file_name_root(oname + ".segreg");
...
/// the analysis
void
AnalysisController::do_analysis(const RPED::MultiPedigree& mp,
...
void
AnalysisController::process_analysis(const PedigreeDataSet& ped_data,
                                     const model&           target_model) const
{
...
// Conditioned Subset in cond_mem_set 
  ...
  {
//cout << "1. commingling_analysis.." << endl;
  ...
  {
//cout << "2. segregation_analysis.." << endl;
...
//cout << "4. Iterative_Models_analysis.." << endl;
...
///
void 
AnalysisController::do_commingling_analysis(const PedigreeDataSet& ped_data, 
...
///
void 
AnalysisController::do_segregation_analysis(const PedigreeDataSet& ped_data, 
                                            model&                 test_model) const
{
  Iterative_Models ma(ped_data, my_out, my_analysis);
...
///
void AnalysisController::do_reg_analysis(const PedigreeDataSet& ped_data, 
                                         model&                 test_model) const
{
...
///
void
AnalysisController::print_analysis_header(const std::string& title) const
{
  my_out.messages()
...
AnalysisController::do_single_evaluation(const PedigreeDataSet& ped_data,
...
void
AnalysisController::produce_allfixed_output(vector<std::pair<string, double> > name_val,
                                            vector<string> par_type, double value, string file) const
{
