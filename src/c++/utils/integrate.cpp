...
xIntgRes* DQAGS(xIntgInp *Xp_i,
			double *result, double *abserr, int *neval, int *ier,
			int *limit, int *lenw, int *last, int *iwork, double *work);
xIntgRes* DQAGI(xIntgInp *Xp_i, double *bound, int *inf,
				 double *result, double *abserr, int *neval, int *ier,
				 int *limit, int *lenw, int *last,
				 int *iwork, double *work);
xIntgRes* call_dqags(xIntgInp *Xp_i);
xIntgRes* call_dqagi(xIntgInp *Xp_i, int N_inf, double R_bound);

xIntgRes* integrate(xIntgInp *Xp_inp, char B_stopOnErr/*=0*/)
{
//	f <- match.fun(f)
//	ff <- function(x) f(x, ...)
	double R_inf = numeric_limits<double>::infinity();
	xIntgRes *Xp_r = NULL;

	wsUint N_limit = Xp_inp->N_subDiv;
	if (N_limit < 1 || (Xp_inp->R_absTol <= 0 && Xp_inp->R_relTol < max(50 * numeric_limits<double>::epsilon(), 
			5e-29))) 
		halt("invalid parameter values");
	if (Xp_inp->R_lower != R_inf && Xp_inp->R_upper != R_inf) {
		Xp_r = call_dqags(Xp_inp);
	} else {
		int		N_inf	= 2;
		double	R_bound	= 0.0;

		if (Xp_inp->R_lower != R_inf) {
			N_inf	= 1;
			R_bound	= Xp_inp->R_lower;
		} else if (Xp_inp->R_upper != R_inf) {
			N_inf	= -1;
			R_bound	= Xp_inp->R_upper;
		}
		Xp_r = call_dqagi(Xp_inp, N_inf, R_bound);
	}
	...
	if (Xp_r->N_err == 6 || (Xp_r->N_err > 0 && B_stopOnErr))
		halt("Integration failed : %s", Sa_msg[Xp_r->N_err]);

	return Xp_r;
}

/* This is *the* ``integr_fn f'' used when called from R : */
static void Rintfn(double *x, int n, xIntgInp *tgt)
{
    int i;
	DEALLOC(tgt->Ra_X);
	wsAlloc(tgt->Ra_X, double, n);
    for (i=0 ; i<n ; i++)
		tgt->Ra_X[i] = x[i];

	tgt->H_func(tgt->Ra_X, n, tgt);
    for (i=0 ; i<n ; i++) {
		x[i] = tgt->Ra_X[i];
		if (x[i] == numeric_limits<double>::infinity() ||
			x[i] == numeric_limits<double>::quiet_NaN())
			halt("non-finite function value");
    }
    return;
}
...
xIntgRes* call_dqags(xIntgInp *Xp_i)
{
    double result, abserr, *work = NULL;
    int neval, ier, limit, lenw, last, *iwork = NULL;

	limit = Xp_i->N_subDiv;
    lenw = 4 * Xp_i->N_subDiv;
	wsAlloc(iwork, int, Xp_i->N_subDiv);
	wsAlloc(work, double, lenw);

    xIntgRes *Xp_ret = DQAGS(Xp_i, &result,
...
{
    double	result, abserr, *work = NULL;
    int		neval, ier, lenw, last, *iwork = NULL;

    lenw = 4 * Xp_i->N_subDiv;
	wsAlloc(iwork, int, Xp_i->N_subDiv);
	wsAlloc(work, double, lenw);

	xIntgRes *Xa_ret = DQAGI(Xp_i, &R_bound, &N_inf, &result,
	   &abserr,&neval,&ier,&(Xp_i->N_subDiv),&lenw,&last,iwork,work);
	DEALLOC(work);
	DEALLOC(iwork);
	return Xa_ret;
}
...
static xIntgRes* DQAGIE(xIntgInp *Xp_i,
		    double *, int *, int *,
		    double *, double *, int *,
		    int *, double *, double *, double *, double *,
		    int *, int *);

static void DQK15I(xIntgInp *Xp_i,
		    double *, int *, double * , double *,
		    double *, double *, double *, double *);

static xIntgRes* DQAGSE(xIntgInp *Xp_i, double *, double *,
		    double *, double *, int *, double *, double *,
		    int *, int *, double *, double *, double *,
		    double *, int *, int *);

static void DQK21(xIntgInp *Xp_i,
		   double *, double *, double *, double *, double *, double *);

static void DQPSRT(int *, int *, int *, double *, double *, int *, int *);
...
***end prologue  dqagi */
    ...
    *result = 0.;
    *abserr = 0.;
    if (*limit < 1 || *lenw < *limit << 2) return NULL;

    l1 = *limit;
    l2 = *limit + l1;
    l3 = *limit + l2;

    xIntgRes *Xp_doNothing = DQAGIE(Xp_i, bound, inf, limit, result, abserr,
		neval, ier, work, &work[l1], &work[l2], &work[l3], iwork, last);
	if (Xp_doNothing == NULL) {
		xIntgRes *Xp_res = NULL;
		wsAlloc(Xp_res, xIntgRes, 1);
		Xp_res->N_err = *ier;
		Xp_res->N_subDiv = *limit;
		Xp_res->R_absErr = *abserr;
		Xp_res->R_value = *result;
...
static
xIntgRes* DQAGIE(xIntgInp *Xp_i, double *bound, int *inf,
...
/*           ----------------------------- */
    ...
    *abserr = 0.;
    ...
    elist[1] = 0.;
    iord[1] = 0;
    if (Xp_i->R_absTol <= 0. && (Xp_i->R_relTol < fmax2(epmach * 50., 5e-29))) *ier = 6;
    if (*ier == 6) return NULL;
...
/*         determine the interval to be mapped onto (0,1).
    ...
    DQK15I(Xp_i, &boun, inf, &c_b6, &c_b7, result, abserr, &defabs, &resabs);
...
/*           test on accuracy */
    ...
    *last = 1;
    ...
    iord[1] = 1;
    dres = fabs(*result);
    errbnd = fmax2(Xp_i->R_absTol, Xp_i->R_relTol * dres);
...
/*           bisect the subinterval with nrmax-th largest error estimate. */
	...
	b2 = blist[maxerr];
	erlast = errmax;
	DQK15I(Xp_i, &boun, inf, &a1, &b1, &area1, &error1, &resabs, &defab1);
	DQK15I(Xp_i, &boun, inf, &a2, &b2, &area2, &error2, &resabs, &defab2);
...
/*           improve previous approximations to integral
	...
	rlist[maxerr] = area1;
	rlist[*last] = area2;
	errbnd = fmax2(Xp_i->R_absTol, Xp_i->R_relTol * fabs(area));
...
/*           call subroutine dqpsrt to maintain the descending ordering
	...
	DQPSRT(limit, last, &maxerr, &errmax, &elist[1], &iord[1], &nrmax);
	if (errsum <= errbnd) {
	    goto L115;
	}
	if (*ier != 0)	    break;
...
/*           test whether the interval to be bisected next is the
	    ...
	    if (fabs(blist[maxerr] - alist[maxerr]) > sm) {
		continue;
	    }
	    extrap = TRUE;
	    nrmax = 2;
...
/*           perform extrapolation.  L60: */
	...
	*result = reseps;
	correc = erlarg;
	ertest = fmax2(Xp_i->R_absTol, Xp_i->R_relTol * fabs(reseps));
	if (*abserr <= ertest) {
	    break;
	}
...
/*         check validity of limit and lenw. */
    ...
    *result = 0.;
    *abserr = 0.;
    if (*limit < 1 || *lenw < *limit *4) return NULL;
...
/*         prepare call for dqagse. */
	...
	xIntgRes *Xp_doNothing = DQAGSE(Xp_i,
		&(Xp_i->R_lower), &(Xp_i->R_upper), &(Xp_i->R_absTol),
		&(Xp_i->R_relTol), limit, result, abserr, neval, ier,
...
static
xIntgRes* DQAGSE(xIntgInp *Xp_i, double *a, double *b, double *
	     ...
	     iord, int *last)
{
    ...
    double correc = 0.0, erlarg = 0.0, ertest = 0.0, sm = 0.0;
/*
...
/*           ----------------------------------- */
    ...
    oflow = DBL_MAX;
    ierro = 0;
    DQK21(Xp_i, a, b, result, abserr, &defabs, &resabs);
...
/*           bisect the subinterval with the nrmax-th largest error estimate. */
	...
	b2 = blist[maxerr];
	erlast = errmax;
	DQK21(Xp_i, &a1, &b1, &area1, &error1, &resabs, &defab1);
	DQK21(Xp_i, &a2, &b2, &area2, &error2, &resabs, &defab2);
...
static void DQELG(int *n, double *epstab, double *
...
static void  DQK21(xIntgInp *Xp_i, double *a, double *b, double *result,
		    double *abserr, double *resabs, double *resasc)
{
    /* Initialized data */
